...
public class Trap_FallingPlatform : MonoBehaviour
{
    ...
    {
        SetupWaypoints();
        float ramdomDelay = Random.Range(0, .6f);

        yield return new WaitForSeconds(ramdomDelay);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (player != null)
        {
            Invoke(nameof(SwitOffPlatform), fallDelay);
